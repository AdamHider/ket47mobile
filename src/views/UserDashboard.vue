<template>
  <base-layout page-title="Личный кабинет" page-default-back-link="/home" :errorMessage="error">
    <div v-if="isSignedIn" class="user-dashboard-header">
      ...
      <ion-item lines="full" class="avatar-row">
          <ion-avatar>
              <img :src="$store.state.hostname+'img/avatar/'+user.user_avatar_name+'.png'" />
          </ion-avatar>
...
export default  {
  ...
  setup() {
    return {
      create, logIn, exit, location, heart, helpCircle, call, mail, card, cart, notifications, chevronForwardOutline
    }
  },
  data(){
    return {
...
.avatar-row ion-label{
  margin-left: 10px;
  font-weight: 500;
}
.user-dashboard-header ion-avatar{
  width: 70px;
  height: 70px;
}
.user-dashboard-header ion-icon {
  font-size: 24px;
  padding: 12px;
}
